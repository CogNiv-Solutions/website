        ...
        <Stagger className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {painPoints.map((p, i) => (
            <StaggerItem key={p.title} className={i === 4 ? "sm:col-span-2 lg:col-span-1" : undefined}>
              <div className="group flex h-full flex-col justify-between rounded-2xl border border-[#0b0e0d]/10 bg-[#fafaf9] p-6 transition-all duration-300 hover:border-[#0b0e0d]/25 hover:bg-white hover:card-shadow">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#0b0e0d] text-white">
                      <p.icon className="h-5 w-5" strokeWidth={1.8} aria-hidden />
                    </span>
                    <span className="font-mono text-[11px] font-semibold uppercase tracking-wider text-[#b45309]">
                      {p.tag}
                    </span>
                  </div>
                  <h3 className="mt-4 text-[17px] font-semibold text-[#0b0e0d]">
                    {p.title}
                  </h3>
                  <p className="mt-2 text-[14px] leading-relaxed text-[#0b0e0d]/65">
                    {p.description}
                  </p>
                </div>
                <div className="mt-5 border-t border-[#0b0e0d]/8 pt-3 text-[12.5px] font-medium text-[#0b0e0d]/50">
          ...
          <StaggerItem className="sm:col-span-2 lg:col-span-1">
            <div className="flex h-full flex-col justify-between rounded-2xl bg-[#0b0e0d] p-6 text-white card-shadow-dark">
              <div>
                ...
                <p className="mt-4 text-[17px] font-medium leading-snug">
                  “If this happens in your business every week, it’s not an employee issue — it’s a missing workflow.”
                </p>
              </div>
              <a
                href="#solutions"
                className="btn-press mt-6 inline-flex w-fit items-center gap-2 text-[14px] font-semibold text-blue-400 hover:text-blue-300"
              >
