    ...
    body: (
      <dl className="grid grid-cols-1 gap-2 sm:grid-cols-3">
        {[
          ...
          <div key={k} className="rounded-2xl border border-[#ea580c]/20 bg-[#fff7ed] px-4 py-3">
            <dt className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#c2410c]/60">{k}</dt>
      ...
      <ul className="space-y-2">
        {[
          ...
          <li key={t} className="flex items-center justify-between gap-3 rounded-2xl border border-[#0b0e0d]/10 bg-white px-4 py-3">
            ...
            <span className="shrink-0 rounded-full bg-[#fff7ed] px-2.5 py-1 font-mono text-[11px] font-medium text-[#c2410c]">match</span>
          </li>
        ))}
      </ul>
    ),
          ...
          <Reveal>
            <div className="rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 card-shadow-dark md:p-7">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-2xl bg-orange-400/15">
                  <MessageSquareText className="h-5 w-5 text-orange-300" aria-hidden />
                </span>
                <div>
                    ...
                    <li key={s.id} className="flex items-start gap-3 rounded-2xl px-2 py-2">
                      <span
                        className={cn(
                          ...
                          state === "done" && "bg-orange-400 text-[#0b0e0d]",
                      ...
                      <span>
                        <span className={cn("block font-mono text-[11px] uppercase tracking-[0.18em]", state === "idle" ? "text-white/35" : "text-orange-300")}>{s.label}</span>
              ...
              <div className="mt-6 flex flex-col gap-2.5 sm:flex-row">
                {!done ? (
                  <button
                    onClick={run}
                    disabled={running}
                    className="btn-press inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-white px-6 py-3.5 text-[15px] font-semibold text-[#0b0e0d] hover:bg-orange-100 disabled:opacity-70"
                  >
                  ...
                  <div className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-orange-400/15 px-6 py-3.5 text-[15px] font-semibold text-orange-200" role="status">
          ...
          <div className="min-h-[420px] rounded-[1.75rem] border border-white/10 bg-white p-5 text-[#0b0e0d] md:p-7" aria-live="polite">
            ...
            <AnimatePresence mode="popLayout">
              {stages.slice(0, visible).map((s) => (
                ...
                >
                  <p className="mb-2 font-mono text-[11px] uppercase tracking-[0.2em] text-[#ea580c]">{s.label}</p>
                  {s.body}
                </motion.div>
              ))}
            </AnimatePresence>
            {running && (
              <div className="relative mt-2 h-1 overflow-hidden rounded-full bg-[#0b0e0d]/8" aria-hidden>
                <div className="absolute inset-y-0 w-1/2 bg-[#ea580c] animate-shimmer" />
              </div>
            )}
          </div>
        </div>
