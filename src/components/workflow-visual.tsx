        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={cn(
              ...
              transformed ? "bg-[#fff7ed] text-[#c2410c]" : "bg-[#0b0e0d]/5 text-[#0b0e0d]/60"
            )}
          >
            <span className={cn("h-1.5 w-1.5 rounded-full", transformed ? "bg-[#ea580c] animate-pulse-dot" : "bg-[#b45309] animate-pulse-dot")} />
              ...
              >
                <span
                  className={cn(
                    ...
                    transformed ? "bg-[#ea580c] text-white" : "bg-[#0b0e0d] text-white"
                  )}
                >
                ...
                {transformed && (
                  <span className="ml-auto h-1.5 w-1.5 shrink-0 rounded-full bg-[#ea580c] animate-pulse-dot" aria-hidden />
                )}
              </motion.div>
            );
          })}
        ...
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-[#0b0e0d] px-4 py-3.5 text-white">
          ...
          <div className="flex gap-1" aria-hidden>
            {Array.from({ length: manual.length + 2 }).map((_, i) => (
              <span key={i} className={cn("h-1.5 rounded-full transition-all duration-500", i <= step ? "w-5 bg-orange-400" : "w-1.5 bg-white/20")} />
            ))}
          </div>
        </div>
      </div>
