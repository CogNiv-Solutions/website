      ...
      <main id="main" className="pt-24 pb-20 md:pt-32 md:pb-28">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mb-8">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[13px] font-mono text-[#0b0b0c]/50">
            ...
            <span className="text-[#0b0b0c] font-medium">AI Solutions</span>
          </nav>
        </div>
            ...
            {engines.map((engine, idx) => {
              ...
              return (
                <div
                  key={engine.id}
                  id={engine.id}
                  className="rounded-3xl border border-[#0b0b0c]/10 bg-white p-6 sm:p-10 md:p-14 shadow-sm transition-all duration-300 hover:border-[#ff6a00]/30"
                >
                  <div className={cn("grid gap-10 lg:grid-cols-12 items-center", isEven && "lg:grid-flow-dense")}>
                    ...
                    <div className={cn("lg:col-span-7", isEven && "lg:col-start-6")}>
                      <div className="flex items-center gap-3">
                        ...
                        <span className="inline-block h-1 w-1 rounded-full bg-[#0b0b0c]/25" />
                        <span className="font-mono text-[11px] uppercase tracking-wider text-[#0b0b0c]/50">
                          {engine.tag}
                        </span>
                      </div>

                      <h2 className="mt-3 text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight text-[#0b0b0c]">
                        {engine.title}
                      </h2>

                      <p className="mt-4 text-[15px] sm:text-[16px] leading-relaxed text-[#0b0b0c]/75">
                        {engine.description}
                      </p>
                      ...
                      <div className="mt-6 space-y-2.5">
                        {engine.capabilities.map((cap) => (
                          <div key={cap} className="flex items-start gap-2.5">
                            <CheckCircle2 className="h-4 w-4 mt-1 shrink-0 text-[#a84300]" />
                            <span className="text-[14px] text-[#0b0b0c]/80">{cap}</span>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="mt-8 flex flex-wrap items-center gap-3">
                        <Link
                          ...
                          className="btn-press inline-flex items-center gap-2 rounded-full bg-[#fff1e6] px-5 py-2.5 text-[13.5px] font-semibold text-[#a84300] hover:bg-[#ffedd5]"
                        >
                        ...
                        <Link
                          href="/audit"
                          className="btn-press inline-flex items-center gap-2 rounded-full border border-[#0b0b0c]/15 px-5 py-2.5 text-[13.5px] font-medium text-[#0b0b0c] hover:bg-slate-50"
                        >
                    ...
                    <div className={cn("lg:col-span-5", isEven && "lg:col-start-1")}>
                      <div className="rounded-2xl border border-[#0b0b0c]/10 bg-[#f4f5f5] p-6 sm:p-8">
                        <div className="flex items-center justify-between border-b border-[#0b0b0c]/10 pb-4 mb-5">
                          <div className="flex items-center gap-2.5">
                            <div className="grid h-9 w-9 place-items-center rounded-lg bg-white border border-[#0b0b0c]/10 text-[#a84300]">
                        ...
                        <div className="space-y-3">
                          {engine.flow.map((step, sIdx) => (
                            <div key={step} className="flex items-center gap-3">
                              <span className="font-mono text-[11px] text-[#0b0b0c]/40 w-5">
                                0{sIdx + 1}
                              </span>
                              <div className="flex-1 rounded-xl border border-[#0b0b0c]/10 bg-white px-3.5 py-2 text-[13px] font-medium text-[#0b0b0c] shadow-2xs">
                                {step}
                              </div>
                            </div>
                          ))}
        ...
        <section className="border-y border-[#0b0b0c]/10 bg-white py-16 md:py-24 mb-20 md:mb-28">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            <Reveal>
              ...
              <p className="mt-4 max-w-2xl mx-auto text-[15px] sm:text-[16px] text-[#0b0b0c]/70">
            ...
            <div className="mt-12 flex flex-wrap items-center justify-center gap-3 max-w-4xl mx-auto">
              {ecosystemTools.map((tool) => (
                <div
                  key={tool}
                  className="rounded-full border border-[#0b0b0c]/10 bg-[#f4f5f5] px-4 py-2 text-[13.5px] font-medium text-[#0b0b0c] shadow-2xs transition-transform hover:scale-105"
                >
                  {tool}
                </div>
              ))}
